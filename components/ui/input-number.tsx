...
interface InputNumberProps extends Omit<React.ComponentProps<typeof Input>, 'value' | 'onChange'> {
  value: number
...
function InputNumber({ 
  ...
  React.useEffect(() => {
    if (!isFocused) {
      // Solo formatear cuando no está enfocado
      setDisplayValue(formatNumber(value, allowDecimals, maxDecimals))
    }
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Pero en edición, permitimos temporalmente sin comas
  ...
  const handleBlur = () => {
    ...
    setDisplayValue(numValue === 0 ? "" : formatNumber(numValue, allowDecimals, maxDecimals))
  }
